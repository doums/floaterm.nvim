...
local preview_command = fn.executable('bat') and
                          [[COLORTERM=truecolor bat --color=always -H {2} {1}]] or
                          'less {1}'
local fzf_command =
  [[fzf --bind="change:reload:rg --vimgrep --hidden {q}" -d : --with-nth=1,4 \
  --preview-window=right,70%,noborder,~3,+{2}+3/2 --preview="]] ..
    preview_command .. [["]]
local tempfile = nil
...
local function rg()
  ...
  local command = fzf_command .. [[ --expect="]] .. keys .. [[" --header="]] ..
                    cwd_sub .. [[ " > ]] .. tempfile
  local config = {command = command, on_exit = on_exit, name = 'rg', cwd = cwd}
